...
이미지 관리 명령어는 로컬 시스템에 존재하는 이미지를 확인하고 Docker Hub에서 이미지를 다운로드 혹은 삭제를 하기 위해 사용되는 명령어입니다.
...
| 명령어 | 설명 | 옵션 |
|---|---|---|
| docker run <이미지> | 지정한 이미지 이용해 컨테이너 생성 및 실행 |
| docker ps | 현재 실행 중인 컨테이너 목록 확인 |
| docker stop <컨테이너> | 지정한 컨테이너 정지 |
| docker start <컨테이너> | 지정한 컨테이너 시작 |
| docker restart <컨테이너> | 지정한 컨테이너 재시작 |
| docker rm <컨테이너> | 지정한 컨테이너 삭제 |
| docker attach <컨테이너> | 실행 중인 컨테이너에 접근하여 터미널 연결 |
| docker exec -it <컨테이너> <명령어> | 실행 중인 컨테이너에서 지정한 명령어 실행 |

### docker run <이미지>

해당 명령어는 지정한 이미지를 이용해 컨테이너를 생성하고 실행하는 명령어입니다.
...
<tbody>
  <tr>
    <td rowspan="9">docker run &lt;이미지&gt;<br/><br/><br/><br/><br/></td>
    <td rowspan="9">지정한 이미지 이용해 컨테이너 생성 및 실행<br/><br/><br/><br/><br/></td>
    <td>-it</td>
    <td>-i, -t 옵션 합성<br/>키보드 입력을 컨테이너로 전달</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>실행 중인 컨테이너에 접근하여 터미널 연결</td>
    <td></td>
    <td></td>
  </tr>
  <tr>
    <td>docker exec -it &lt;컨테이너&gt; &lt;명령어&gt;</td>
    <td>실행 중인 컨테이너에서 지정한 명령어 실행</td>
    <td></td>
    <td></td>
  </tr>
</tbody>
