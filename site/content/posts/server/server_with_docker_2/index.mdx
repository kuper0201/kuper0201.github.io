...
해당 명령어는 컨테이너들의 목록과 ID를 확인하는 명령입니다.
...
docker run 실행 시 --name 옵션을 통해 컨테이너의 이름을 정해 준 경우에는 이름으로 명령을 수행 할 수 있지만, 그렇지 않은 경우에는 docker ps 명령을 통해 출력된 컨테이너 ID를 통해 명령을 수행해야 합니다.
