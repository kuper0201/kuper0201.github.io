...
이때 두 벡터 사이의 각도 $\theta$에 따라 결과 벡터의 방향의 부호가 결정되게 된다.

 0 < $\theta$ < 180 일때 결과 벡터는 양의 방향을 가진다.
 180 < $\theta$ < 360 일때 결과 벡터는 음의 방향을 가진다.
 $\theta$ = 0 or $\theta$ = 180 일때 결과 벡터는 0이 된다.
...
$$
AB \times AC = (0, 0, (Bx - Ax)(Cy - Ay) - (Cy - Ay)(Bx - Ax))
$$
