...
어떤 정수 X가 1보다 큰 제곱수로 나누어 떨어지지 않을 때, 그 수를 제곱ㄴㄴ수라고 한다.<br/>
...
2. 2부터 $\sqrt{max}$까지 제곱하며 그의 배수를 모두 Set에 저장한다.<br/>예를 들어 (2*2)*1, (2*2)*2, (2*2)*3 ... (2*2)*K의 수들은 모두 제곱수의 배수이므로 Set에 저장한다. 이때 (2*2)*K <= max인 만큼만 반복한다.
