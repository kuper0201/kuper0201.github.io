...
입력의 개수는 500만 이하라고 하였으므로 32bit * 5000000 = 160000000bit = 약 19MB의 메모리가 필요하다는 것을 알 수 있다.
하지만 해당 문제의 메모리 제한은 8MB로 19MB의 메모리를 사용하게 되면 메모리 초과가 발생할 것이다.

이러한 문제를 해결하기 위해서 비트 마스킹(Bit Masking)이라는 기법을 이용할 수 있다.
