...
'에라토스테네스의 체'를 시각화한 이미지는 다음과 같다.

1. 1은 소수가 아니므로 제외한다.
![](./images/table.jpg)

2. 첫 번째 소수인 2의 배수를 제외한다.(2는 제외하지 않는다)

3. 두 번째 소수인 3의 배수를 제외한다.(3은 제외하지 않는다)

4. 세 번째 소수인 5의 배수를 제외한다.(5는 제외하지 않는다)

5. K번째 소수가 $\sqrt{max}$보다 작을 동안 반복하여 제외한다.(K번째 소수는 제외하지 않는다)
