...
하지만 루프의 크기가 50보다 클 경우에는 제한 횟수만큼 상자를 열어봐도 루프를 전부 확인하지 못하고 끝나버릴 것이다.

루프가 생성되는 전체 경우의 수 중 50보다 큰 루프의 크기를 가지지 않을 확률이 약 31%라고 한다. (자세한 계산은 영상에 잘 성명되어 있다)
...
public class Main {
    ...
    static void initBox() {
        ...
        // 상자에 번호 넣기
        for(int i = 0; i < prisoner; i++) box.add(i);
