...
본 프로젝트에서는 다중 분류 모델을 이용한 방식으로 모델을 구축하였습니다. 단순히 말벌과 꿀벌을 분류하고 알림을 주기 위해서는 이진 분류 모델을 구축하는 것이 이상적이지만 본 프로젝트에서는 노이즈 데이터 또한 이용하고 말벌과 서식지가 겹치는 다른 생물들 또한 분류하기를 원했기 때문에 다중 분류 모델을 채택하게 되었습니다.
...
하지만 Bee or Wasp?의 데이터셋은 이미지의 개수가 12000여 장으로 볼륨이 작고 외국의 데이터셋이기 때문에 국내 생물들의 특성은 반영하지 못하는 경향이 커 보였습니다. 따라서 이미지 크롤링과 이미지 증강 기법을 통해 30000여 장으로 데이터셋의 볼륨을 키워 재구축 하였습니다.
...
모델의 성능을 분석해 보겠습니다. 처음 이미지는 검증 데이터셋을 이용한 모델의 평가 수치입니다.
...
구축한 모델이 제대로 예측을 하는지 예측 결과를 확인해 보겠습니다. 약 51개의 데이터를 예측하였고 각 이미지 아래에 있는 라벨은 모델이 예측한 값을 의미합니다. 모델의 예측이 맞았을 경우 청색으로 출력되게 하였고 틀렸을 경우에는 적색으로 출력되게 하였습니다.
